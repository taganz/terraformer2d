...
enum LOGEVENT {
	...
	INFO_CREATURE,					// info about creature parameters
...
function log_event_to_string(_log_event) {
	...
	switch (_log_event) {
		...
		case LOGEVENT.INFO_CREATURE:			_msg = "DEBUG INFO CREATURE"; break;
	}
	return _msg;
}
...
#region var init
	...
	log_simulation_event = function(_event, _id1, _arg1, _arg2, _arg3) {
		...
		switch(_event) {
			...
			case LOGEVENT.CREATURE_DEBUG: {
				...
				if (LOG_CREATURES_ALL
					...
					_col_biomass = string(units_to_kg(_id1.structure.biomass));
					...
					if _event == LOGEVENT.CREATURE_BIOMASS_DIGESTIVE {
						...
						_col_biomass = string(units_to_kg(_arg1));	
					...
					if _event == LOGEVENT.INFO_CREATURE {
		...
		if _do_log {
			_lines_buffer [_line] = 
					string(controller.time.current_sim_step)
					+ CSV_SEPARATOR
					+ string(controller.time.current_sim_month)
					+ CSV_SEPARATOR
					+ string(controller.time.current_sim_year)
					+ CSV_SEPARATOR
