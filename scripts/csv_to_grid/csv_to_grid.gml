...
//RD:
function csv_to_grid (_filename) {
...
//RD var _filename         = argument[0];
var _force_strings    = false;

// RD 21/2/21
//RD var _cell_delimiter   = chr(44); //comma
var _cell_delimiter   = chr(59); //semicolon

// RD 21/2/21
var _decimal_comma = true;   // allow decimal comma  (needs semicolon _cell_delimiter)
...
for( var _i = 0; _i < _size; _i++ ) {
	...
	if ( ( _val == _cell_delimiter_ord ) or ( _newline ) ) and ( !_in_string ) {
		...
		// RD 21/2/21 - if _str is "", real(_str) would raise an error
		if (_str == "") _is_decimal = false;
	...
	//RD - convert decimal "," to "."
	if (_decimal_comma)
		if (_is_decimal) and _val==44    // comma
			_val = 46					// dot
